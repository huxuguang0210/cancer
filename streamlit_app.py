...
model_dir = "results_clinical_enhanced_v3"
...
print(f"Scaler类型: {type(prep.scaler).__name__}")
print(f"期望特征数: {prep.scaler.n_features_in_}")

if hasattr(prep.scaler, 'center_'):
    print(f"RobustScaler中位数: {prep.scaler.center_[:5]}... (前5个)")
    print(f"RobustScaler IQR: {prep.scaler.scale_[:5]}... (前5个)")
elif hasattr(prep.scaler, 'mean_'):
    print(f"StandardScaler均值: {prep.scaler.mean_[:5]}... (前5个)")

if hasattr(prep, 'selected_features') and prep.selected_features:
    print(f"选择的特征: {prep.selected_features}")
else:
    print("未进行特征选择")

# 2. 检查ds_min_max
print("\n" + "="*60)
print("2. DeepSurv归一化范围")
print("="*60)
ds_mm = np.load(f"{model_dir}/ds_min_max.npy")
print(f"min_ds: {ds_mm[0]:.4f}")
print(f"max_ds: {ds_mm[1]:.4f}")
print(f"范围: {ds_mm[1] - ds_mm[0]:.4f}")
...
print(f"时间点数: {len(time_cuts)}")
print(f"时间范围: [{time_cuts[0]:.1f}, {time_cuts[-1]:.1f}]")
...
print("="*60)
try:
    df = pd.read_csv("data_original2024_extend_normalization_english.csv")
    feat_cols = [c for c in df.columns if c not in ['time', 'event', 'id', 'Unnamed: 0']]
...
try:
    # 创建一个测试输入
    test_input = np.zeros((1, prep.scaler.n_features_in_))
    test_output = prep.transform(test_input)
    print(f"输入全0时的输出范围: [{test_output.min():.4f}, {test_output.max():.4f}]")
    
    test_input = np.ones((1, prep.scaler.n_features_in_))
    test_output = prep.transform(test_input)
    print(f"输入全1时的输出范围: [{test_output.min():.4f}, {test_output.max():.4f}]")
except Exception as e:
    print(f"测试失败: {e}")
